...
//functions to handle DV 1-3
export function taskToString(taskID)
{
	var name;
...
export function calErrors() {
  sessionStorage["errorCount"] = parseInt(sessionStorage["errorCount"])+1;
}

export function sendEvent(evntName,custom) {
  // console.log('sendClickEvent()');
  var timeEnd = (new Date()).getTime();
  loggingjs.logEvent(null, evntName, {
    eventName: evntName,
    info: {
      'timeStart': sessionStorage.getItem("timeStart"), 
      'timeEnd': timeEnd, 
      'clickCount': sessionStorage.getItem("clickCount"),
      'errorRate': sessionStorage.getItem("errorCount")
    }});
}
// end DV handle
...
function logEvent(event, customName, customInfo) {
  ...
  var timeEnd = (new Date()).getTime();
  var timeStart=sessionStorage.getItem("timeStart");
  time = (timeEnd- timeStart)/1000;
  /*if(eventName ==='click'){
	  //customInfo.info['timeEnd']=timeEnd;
      time = (timeEnd- timeStart)/1000;
  }else{
  ...
  var iv1=sessionStorage.getItem("iv1");
  var iv2=sessionStorage.getItem("iv2");
  var iv3=sessionStorage.getItem("iv3");
  ...
  if (ENABLE_CONSOLE_LOGGING) {
    console.log(uid, time, task,iv1,iv2,iv3, eventName, target, info, state, LOG_VERSION);
  }
  if (ENABLE_NETWORK_LOGGING) {
    sendNetworkLog(uid, time,task,iv1,iv2,iv3, eventName, target, info, state, LOG_VERSION);
  }
}
...
//
/////////////////////////////////////////////////////////////////////////////
// user events submission function
// submits to the google form at this URL:
// docs.google.com/forms/d/e/1FAIpQLScjVPPPKnGZOGjS5xFCkr_TbrxGhE1NxKihNi38s0DMlX7Spg/viewform?usp=sf_link
